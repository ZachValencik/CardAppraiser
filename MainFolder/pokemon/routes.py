...
def myList():
    if "user" in session:  # if user is logged in it will render the pokemon cards, otherwise redirect to login
        user = session["user"]
        return render_template('myList.html', title='myList')
    else:
        return redirect(url_for('login'))
    ...
    if "user" in session:
        ...
        if request.method == 'POST':
            ...
            if len(mediaPost) < 1:
                ...
                mysql.connection.commit()
                cur.close()
                flash(f'Your post was Updated',
                      'success')  # A flash method that alerts the user that their post was completed
        ...
        if request.method == 'POST':
            ...
            if len(mediaPost) < 1:
                ...
                if (image.filename == ''):
                    ...
                    cur.execute("INSERT INTO SocialMedia(post,username,image) VALUES(%s,%s,%s)",
                                (mediaPost, user, image.filename))

                mysql.connection.commit()
                cur.close()
                flash(f'Your post was published',
                      'success')  # A flash method that alerts the user that their post was completed
                # return render_template('socialMedia.html',title='Pokemon Forum', userName=user, dataMediaPosts=dataMediaPosts)
